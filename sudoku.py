...
from digit_recognizer import image
import cv2
...
def update():
    for r in range(9):
        for t in range(9):
            ...
            if(w==0):
                w="  "
                l=Button(my_frame,text=str(w),font=("Helvetica", 16), relief="ridge",command=lambda row=r,column =t :check_if(row,column))
                l.config(width=2)
                  
                l.grid(row=r+count,column=t)
                
            else:

                l=Button(my_frame,text=str(w),font=("Helvetica", 16),relief="raised",command=lambda row=r,column =t :wrong_input(row,column))
                l.config(width=2)
...
def wrong_input(r, c):
    ...
    if(q is None):
        ...
        status.set("your input has been recorded")
        status_bar.config(borderwidth=3,background="magenta",foreground="black")
        arr[r][c]=q
        update()
...
menu.add_command(label="save current ",command =copy_to_b)
